...
#
OPTD_POR_FILE=${TMP_DIR}${OPTD_POR_FILENAME}

##
# Path of the Nice data store
OPTD_SCHEDULE_PATH=/remote/oridatacenter/PROCESSED/oag_ssim7/all_catalog
...
LATEST_EXTRACT_DATA=`ls ${OPTD_SCHEDULE_PATH} | grep "${SCH_FILE_PFX1}" | tail -1 | sed -e "s/${SCH_FILE_PFX1}\([0-9]\+\)\.txt/\1/"`
if [ "${LATEST_EXTRACT_DATA}" = "" ]
then
	LATEST_EXTRACT_DATA=`ls ${OPTD_SCHEDULE_PATH} | grep "${SCH_FILE_PFX2}" | tail -1 | sed -e "s/${SCH_FILE_PFX2}\([0-9]\+\)\.csv\.bz2/\1/"`
fi
...
bzcat ${CSV_SCH_FILE} | awk -F'^' -f ${OPT_EXTRACTER} > ${SPE_CSV_OUT_APT_PAL_FILE}
...
awk -F'^' -f ${FREQ_COUNTER} ${CSV_OUT_APT_PAL_FILE_TMP} > ${CSV_OUT_APT_PAL_FILE_TMP2}
...
${CITY_ADDER} ${SNAPSHOT_DATE} ${OPTD_POR_FILENAME} 0 || exit -1
...
sed -e "s/^\([A-Z0-9]\{2\}\)\^\(.*\)$/ALL\^\2/g" ${CSV_OUT_APT_PAL_FILE} > ${CSV_OUT_APT_MGD_FILE_TMP}
sed -e "s/^\([A-Z0-9]\{2\}\)\^\(.*\)$/ALL\^\2/g" ${CSV_OUT_ALL_PAL_FILE} > ${CSV_OUT_ALL_MGD_FILE_TMP}
...
awk -F'^' -f ${FREQ_COUNTER} ${CSV_OUT_APT_MGD_FILE_TMP2} > ${CSV_OUT_APT_MGD_FILE_TMP3}
sort -t'^' -k2,3 ${CSV_OUT_APT_MGD_FILE_TMP3} > ${CSV_OUT_APT_MGD_FILE}
awk -F'^' -f ${FREQ_COUNTER} ${CSV_OUT_ALL_MGD_FILE_TMP2} > ${CSV_OUT_ALL_MGD_FILE_TMP3}
...
echo "wc -l ${CSV_OUT_APT_PAL_FILE} ${CSV_OUT_ALL_PAL_FILE}"
echo
echo "Merged all the airline specific entries:"
echo "wc -l ${CSV_OUT_APT_MGD_FILE} ${CSV_OUT_ALL_MGD_FILE}"
echo
