...
# for any given language, from the OPTD-maintained data files of POR
# (i.e., in ../opentraveldata/, optd_por_public.csv and optd_airline_por.csv).
#
...
BEGIN {
    ...
    awk_file = "extract_por_per_airline.awk"
    ...
    # Fields for the list of POR file with airline codes
    K_AIR_ISO = 1
    K_AIR_POR_ORG = 2
    K_AIR_POR_DST = 3
	K_AIR_FLT_FREQ = 4
...
##
# OPTD-maintained list of POR with airline codes
#
# Sample input lines:
...
# QF^ADL^BNE^1462
#
/^([A-Z0-9]{2})\^([A-Z0-9]{3})\^([A-Z0-9]{3})\^([0-9]{1,9})$/{
    # 2-char ISO of the airline
    air_code = $K_AIR_ISO

    # IATA code of the origin POR (point of reference)
    air_por_org = $K_AIR_POR_ORG

    # IATA code of the destination POR (point of reference)
    air_por_dst = $K_AIR_POR_DST

	# Number of flights (flight frequency) for that (origin, destination) pair
	air_flt_freq = $K_AIR_FLT_FREQ
	
	# Register the two POR for that airline
	registerPORAirlineLine(air_code, air_por_org, air_por_dst, air_flt_freq)
}
...
/^([A-Z0-9]{3})\^([A-Z0-9]{0,4})\^([A-Z0-9]{0,4})\^/{
	...
	# Retrieve the accumulated flight frequency for that (airline, POR) pair
	air_por_flt_freq = getAirlinePORFltFreq(tgt_air, iata_code)
