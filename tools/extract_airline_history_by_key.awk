...
BEGIN {
    ...
    awk_file = "extract_airline_history_by_key.awk"
...
/^[a-z]{3}-[a-z0-9\-]+\^[0-9]*\^([0-9]{4}-[0-9]{2}-[0-9]{2})?\^([0-9]{4}-[0-9]{2}-[0-9]{2})?\^([A-Z0-9]{3})?\^([A-Z0-9*]{2})?\^/ {
	...
	full_record = optd_air_list[key]
	if (!full_record) {
		optd_air_list[key] = output_line

	} else {
		optd_air_list[key] = full_record "\n" output_line
		optd_air_hist_list[key] = 1
	}

	# DEBUG
	# if (version != "1") {
	#	print ("[N-1][" key "] " full_record)
	#	print ("[" version "][" key "] " output_line)
	#}
}
...
END {
	...
	for (air_key in optd_air_hist_list) {
		print (optd_air_list[air_key])
	}
}
