...
BEGIN {
    ...
    awk_file = "extract_por_for_a_language_with_por_list.awk"
	...
	# POR list on which the translations are sought
	delete por_filter_list
...
# QYG
/^([A-Z0-9]{3})$/{
    # IATA code of the point of reference (POR)
    iata_code = $1

	# Register the POR in the filter list
	por_filter_list[iata_code] = 1
}
...
/^([A-Z0-9]{3})\^([A-Z0-9]{0,4})\^([A-Z0-9]{0,4})\^/{
	...
	if (env_id == "" && por_filter_list[iata_code] == 1) {
		print (outputLine)
	}
}
